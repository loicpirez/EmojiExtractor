...
def extract_images(emojiClass, args):
    ...
    if emojiClass.rawApplePng is not "missing":
        extract_base64(emojiClass.rawApplePng, name, "emoji/apple/", "png", args)
    if emojiClass.rawGooglePng is not "missing":
        extract_base64(emojiClass.rawGooglePng, name, "emoji/google/", "png", args)
    if emojiClass.rawTwitterPng is not "missing":
        extract_base64(emojiClass.rawTwitterPng, name, "emoji/twitter/", "png", args)
    if emojiClass.rawEmojiOnePng is not "missing":
        extract_base64(emojiClass.rawEmojiOnePng, name, "emoji/emojione/", "png", args)
    if emojiClass.rawFacebookPng is not "missing":
        extract_base64(emojiClass.rawFacebookPng, name, "emoji/facebook/", "png", args)
    if emojiClass.rawMessengerPng is not "missing":
        extract_base64(emojiClass.rawMessengerPng, name, "emoji/messenger/", "png", args)
    if emojiClass.rawSamsungPng is not "missing":
        extract_base64(emojiClass.rawSamsungPng, name, "emoji/samsung/", "png", args)
    if emojiClass.rawWindowsPng is not "missing":
        extract_base64(emojiClass.rawWindowsPng, name, "emoji/windows/", "png", args)
    if emojiClass.rawGmailGif is not "missing":
        extract_base64(emojiClass.rawGmailGif, name, "emoji/gmail/", "gif", args)
    if emojiClass.rawSoftBankGif is not "missing":
        extract_base64(emojiClass.rawSoftBankGif, name, "emoji/softbank/", "gif", args)
    if emojiClass.rawDocomoGif is not "missing":
        extract_base64(emojiClass.rawDocomoGif, name, "emoji/docomo/", "gif", args)
    if emojiClass.rawKDDIGif is not "missing":
        extract_base64(emojiClass.rawKDDIGif, name, "emoji/kddi/", "gif", args)


def emojilist(args):
    print_verbose("Downloading emoji list from unicode.org...\n", args)
    content = urlopen("http://www.unicode.org/emoji/charts/full-emoji-list.html").read()
